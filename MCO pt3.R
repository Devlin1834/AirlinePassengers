...
ggplot(mco.lag, aes(x = LAG, y = INT)) + 
	 ...
	 geom_point() + geom_smooth(method = loess, se = TRUE, size = 1, col = 'darkorange1') + 
...
### Graph 10: Lag with DOM and INT ###
mco.lag[3] <- as.data.frame(mco$DOMESTIC)[-1,]
mco.lag[4] <- as.data.frame(zlag(mco$DOMESTIC))[-1,]
colnames(mco.lag) <- c('INT', 'I.LAG', 'DOM', 'D.LAG')
...
plotable[3] <- lags[2]
plotable[4] <- lags[3]
colnames(plotable) <- c('BASE', 'Pass', 'LAG', 'L.Pass')
ggplot(plotable, aes(x = L.Pass, y = Pass, color = BASE)) + 
	 labs(x = 'Last Period\'s Passengers', y = 'This Period\'s Passengers',
		title = 'Passengers with a Lag', color = 'Origin') + 
	 geom_point(shape = 15) + scale_color_manual(values = c('mediumblue', 'firebrick')) +
	 mco_theme + theme(panel.grid.major.y = element_line(color = 'darkorange1', linetype = 2))
